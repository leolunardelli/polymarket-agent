...
export class SentimentAnalyzer {
  private positiveWords = new Set(['good', 'great', 'excellent', 'amazing', 'bullish', 'moon', 'pump', 'win', 'success', 'profit', 'gains', 'strong', 'surge', 'rally', 'boom', 'growth', 'optimistic', 'positive']);
  private negativeWords = new Set(['bad', 'terrible', 'awful', 'horrible', 'bearish', 'dump', 'crash', 'lose', 'fail', 'loss', 'weak', 'decline', 'drop', 'fall', 'collapse', 'pessimistic', 'negative', 'concern']);
  private intensifiers = new Set(['very', 'extremely', 'highly', 'really', 'super', 'incredibly', 'absolutely', 'totally', 'completely']);
  private negators = new Set(['not', 'no', 'never', 'neither', 'hardly', "don't", "doesn't", "can't", "couldn't"]);
  ...
  analyzeSentiment(text: string): SentimentResult {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (word_score !== 0) {
        if (i > 0 && this.intensifiers.has(words[i - 1])) word_score *= 1.5;
        if (i > 0 && this.negators.has(words[i - 1])) word_score *= -1;
        score += word_score;
  ...
  aggregateSentiment(data: SocialData[], decay_hours: number = 24): AggregatedSentiment {
    if (data.length === 0) {
      return {
        overall_score: 0,
        confidence: 0,
  ...
  private getSourceWeight(source: 'twitter' | 'reddit' | 'news'): number {
    switch (source) {
      case 'news':
        return 1.5;
      case 'twitter':
  ...
  calculateSentimentMomentum(historical: AggregatedSentiment[]): { momentum: number; acceleration: number; direction: 'IMPROVING' | 'DECLINING' | 'STABLE' } {
    if (historical.length < 2) return { momentum: 0, acceleration: 0, direction: 'STABLE' };
    
    const scores = historical.map(h => h.overall_score);
    let momentum = 0;
    for (let i = 1; i < scores.length; i++) momentum += scores[i] - scores[i - 1];
  ...
  detectAnomalies(historical: AggregatedSentiment[]): { is_anomaly: boolean; z_score: number; significance: 'LOW' | 'MEDIUM' | 'HIGH' } {
    if (historical.length < 3) return { is_anomaly: false, z_score: 0, significance: 'LOW' };
  ...
  generateSentimentSignal(
    ...
    historical: AggregatedSentiment[]
  ): {
