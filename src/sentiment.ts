...
export class SentimentAnalyzer {
  ...
  private intensifiers = new Set(['very', 'extremely', 'highly', 'really', 'super', 'incredibly', 'absolutely', 'totally', 'completely', 'overwhelmingly', 'significantly']);
  private negators = new Set(['not', 'no', 'never', 'neither', 'hardly', "don't", "doesn't", "can't", "couldn't", "won't", "unlikely"]);

  // P3 #3: Bigram / multi-word phrase scoring
  private positiveBigrams = new Map<string, number>([
  ...
  private negativeBigrams = new Map<string, number>([
  ...
  analyzeSentiment(text: string): SentimentResult {
    ...
    for (const [phrase, val] of this.positiveBigrams) {
      if (lower.includes(phrase)) { score += val; magnitude += Math.abs(val); count++; }
    }
