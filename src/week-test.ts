...
interface MarketData {
  ...
  liquidity: number;
  endDate: string;
  // Enrichment fields (P0 #2, #3, P1 #1, #2, P2 #2)
  momentum?: number;
  spread?: number;
...
class WeekTest {
  ...
  private categoryPositionCount: Map<string, number> = new Map();
  ...
  private recentlyClosed: Map<string, number> = new Map(); // conditionId -> closeTimestamp
    ...
    if (savedState && resume) {
      ...
      if (savedState.peakPrices) {
        for (const [key, price] of Object.entries(savedState.peakPrices)) {
          this.marketPriceCache.set(key, { price: price as number, timestamp: Date.now() });
        }
      }
   ...
   */
  private async enrichMomentum(market: MarketData): Promise<void> {
    try {
      if (!market.tokenIds || market.tokenIds.length === 0) return;
   ...
   */
  private async enrichSpread(market: MarketData): Promise<void> {
    try {
      if (!market.tokenIds || market.tokenIds.length === 0) return;
   ...
   */
  private enrichSentiment(market: MarketData): void {
   ...
   */
  private async enrichTags(market: MarketData): Promise<void> {
    // Use the question text heuristic for category tagging
    const q = market.question.toLowerCase();
    const tags: string[] = [];
    if (/election|president|vote|democrat|republican|congress|senate/i.test(q)) tags.push('politics');
    if (/nfl|nba|mlb|nhl|soccer|football|basketball|tennis|sports/i.test(q)) tags.push('sports');
    if (/bitcoin|ethereum|crypto|btc|eth|token|blockchain/i.test(q)) tags.push('crypto');
    if (/gdp|inflation|fed|interest rate|cpi|recession|economy/i.test(q)) tags.push('economics');
    if (/ai|artificial intelligence|openai|gpt|technology|tech/i.test(q)) tags.push('tech');
  ...
  private async executeTrade(market: MarketData, analysis: ReturnType<typeof this.analyzeMarket>): Promise<void> {
    ...
    if (availableForTrading <= 0) {
      logger.debug('Cash reserve limit reached, skipping trade', {
    ...
    if (categoryCount >= CONFIG.maxPositionsPerCategory) {
      logger.debug('Category limit reached', { tag: primaryTag, count: categoryCount });
      return;
    }
    ...
    try {
      ...
      logger.info('Virtual trade executed', {
        side: analysis.side,
  ...
  private async checkExitSignals(markets: MarketData[]): Promise<void> {
    ...
    for (const position of portfolio.positions) {
      ...
      const exitSignal = this.strategy.shouldExit(
        { ...position, peakPrice },
        snapshot,
      );
      ...
      if (exitSignal.shouldExit) {
        ...
        try {
          ...
          const sellQty = Math.max(
            1,
            Math.floor(position.quantity * exitSignal.exitQuantityPercent / 100),
          );
          ...
          if (sellQty >= position.quantity) {
            ...
            if (count > 0) this.categoryPositionCount.set(tag, count - 1);
  ...
  private async runCycle(): Promise<void> {
    try {
      ...
      if (currentDrawdown >= CONFIG.maxDrawdownPercent) {
        logger.warn('CIRCUIT BREAKER: Max drawdown reached, no new trades', {
      ...
      if (maxNewPositions > 0 && hasCashForTrading) {
        ...
        const existingConditionIds = new Set(
          portfolio.positions.map(p => p.tokenId.split('_')[0])
        );
        
        // Count existing low-price positions for concentration limit
        const lowPriceCount = portfolio.positions.filter(
          p => p.entryPrice < CONFIG.lowPriceThreshold
        ).length;
        ...
        for (const market of toEnrich) {
          ...
          // Duplicate check
          if (existingConditionIds.has(market.conditionId)) continue;
